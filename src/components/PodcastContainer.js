...
const PodcastContainer = () => {
  ...
  const fetchPodcast = async (pageToken) => {
    if (loading) return;
    setLoading(true);
    setError("");
    let url = PODCAST_API;
    ...
    try {
      ...
      if (!res.ok) {
        let message = "Failed to load podcast";
        try {
  ...
  const handleScroll = (e) => {
    if (loading || podcast.length === 0) return;
  ...
  return (
    <div className="h-screen overflow-y-auto bg-gray-50 dark:bg-gray-900 dark:text-gray-100" onScroll={handleScroll}>
      ...
      {!loading && !error && podcast.length === 0 && (
